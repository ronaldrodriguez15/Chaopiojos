...
const PublicBooking = () => {
  ...
  const [sellerReferralContext, setSellerReferralContext] = useState({
    isActive: false,
    token: "",
    businessName: "",
    sellerName: "",
        ...
        if (isMounted && normalized.length) {
          ...
          setForm((prev) => ({
            ...
            servicesPerPerson: prev.servicesPerPerson?.length
              ? prev.servicesPerPerson.map(
                  (service) => service || normalized[0]?.value || "Normal",
                )
  ...
  useEffect(() => {
    ...
    const loadBookingSettings = async () => {
      ...
      if (isMounted && result.success) {
        ...
        const nextTemplate =
          result.settings?.whatsappConfirmationTemplate ||
          DEFAULT_WHATSAPP_CONFIRMATION_TEMPLATE;
  ...
  useEffect(() => {
    const onSettingsUpdated = (event) => {
      ...
      if (
        typeof event?.detail?.whatsappConfirmationTemplate === "string" &&
        event.detail.whatsappConfirmationTemplate.trim()
      ) {
    ...
    const onStorage = (event) => {
      ...
      if (
        event.key === "booking_whatsapp_template" &&
  ...
  useEffect(() => {
    ...
    const forwardScrollToParent = (deltaY) => {
      ...
      try {
        if (
          ...
          typeof window.parent.scrollBy === "function"
        ) {
    ...
    const handleTouchMove = (event) => {
      ...
      if (typeof currentY !== "number" || touchStartYRef.current === null)
        return;
    ...
    try {
      ...
      if (result.success && result.data.valid) {
        ...
        toast({
          ...
          className:
            "bg-green-100 border-2 border-green-200 text-green-800 rounded-2xl font-bold",
        });
      } else {
        setReferralValidation({
          isValid: false,
  ...
  const handleGoToBold = async () => {
    ...
    if (
      !Array.isArray(form.servicesPerPerson) ||
      form.servicesPerPerson.length === 0
    ) {
      toast({
        title: "⚠️ Servicio requerido",
        description:
          "Selecciona al menos un servicio antes de generar el pago.",
        duration: 4000,
        variant: "destructive",
      });
      return;
    ...
    try {
      if (boldCheckoutSession?.url) {
        const reopenedWindow = window.open(
          boldCheckoutSession.url,
          "_blank",
          "noopener,noreferrer",
        );
        if (!reopenedWindow) {
      ...
      if (!result.success || !result.url) {
        ...
        toast({
          ...
          description:
            result.message || "No fue posible generar el link de pago.",
          duration: 5000,
          variant: "destructive",
        });
        return;
  ...
  const handleSubmit = async (event) => {
    ...
    if (!form.direccion || form.direccion.trim() === "") {
      errors.direccion =
        "La dirección es obligatoria para realizar el servicio";
    }
    ...
    if (requireAdvance12h) {
      ...
      const [slotHour, slotMinute] = selectedSlot
        .split(":")
        .map((v) => parseInt(v, 10));
      ...
      if (selectedDateTime.getTime() - now.getTime() < twelveHoursMs) {
        toast({
          ...
          description:
            "Los servicios se solicitan con mnimo 12 horas de antelacin.",
          duration: 4000,
          variant: "destructive",
        });
        return;
    ...
    if (
      ...
      !boldCheckoutSession?.paymentLinkId
    ) {
      toast({
        title: "⚠️ Pago pendiente",
        description:
          "Debes generar el link de Bold antes de confirmar la reserva.",
        duration: 4000,
        variant: "destructive",
      });
      return;
    ...
    try {
      ...
      const bookingData = {
        ...
        paymentLinkId:
          form.paymentMethod === "pay_now"
            ? boldCheckoutSession?.paymentLinkId || null
            : null,
        referralCode: referralValidation.isValid ? referralCode : null, // Enviar código si es válido
        sellerReferralToken: sellerReferralContext.isActive
          ? sellerReferralContext.token
          : null,
      };
      ...
      toast({
        title: "❌ Error inesperado",
        description:
          "Ocurrió un problema al procesar tu reserva. Inténtalo nuevamente.",
        duration: 4000,
        variant: "destructive",
      });
    } finally {
  ...
  const whatsappConfirmationMessage = useMemo(() => {
    ...
    const servicesList =
      Array.isArray(confirmedBooking.servicesPerPerson) &&
      confirmedBooking.servicesPerPerson.length
        ? confirmedBooking.servicesPerPerson
            .map((service, idx) => `   ${idx + 1}. ${service}`)
            .join("\n")
        : "";
  ...
  return (
    <>
      <div className="min-h-0 md:min-h-screen bg-[#8bb6d9] font-sans text-gray-800 text-[20px] md:text-xl leading-relaxed md:leading-normal relative overflow-hidden flex items-start md:items-center justify-center px-2 md:px-6 py-2 md:py-6">
          ...
          <div className="relative max-w-6xl w-full mx-auto">
            <div className="mb-3 md:mb-6 rounded-[1.5rem] md:rounded-[2rem] border-4 border-cyan-100 bg-gradient-to-r from-white via-cyan-50 to-blue-50 p-3 md:p-6 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] md:text-sm font-black uppercase tracking-[0.22em] md:tracking-[0.28em] text-cyan-600">
                      Agenda Oficial
                    </p>
            ...
            {!showConfirmation ? (
              <div className="bg-white rounded-2xl md:rounded-[2rem] p-5 md:p-8 shadow-xl space-y-5 md:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs md:text-sm font-bold text-gray-500 mt-1 capitalize">
                      {monthLabel}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-7 gap-2 md:gap-3 text-center font-bold text-gray-500">
                  {WEEK_DAYS.map((day) => (
                    <div
                      key={day}
                      className="uppercase tracking-wide text-xs md:text-sm text-gray-400"
                    >
                      {day}
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-7 gap-2 md:gap-3">
                  {calendarDays.map((dayInfo) => {
                    ...
                    const hasSlots =
                      slots.length > 0 && isCurrentMonth && !isPast;
                    const isSelected = selectedDate
                      ? key === buildDateKey(selectedDate)
                      : false;
                    ...
                    const cellClasses = [
                      ...
                      isToday
                        ? "shadow-md md:shadow-lg border-orange-300 ring-1 md:ring-2 ring-orange-200 ring-offset-1 md:ring-offset-2"
                        : "",
                      hasSlots
                        ? "cursor-pointer hover:-translate-y-0.5 md:hover:-translate-y-1 hover:shadow-md md:hover:shadow-lg hover:border-orange-400 active:translate-y-0"
                        : "cursor-default",
                      isSelected
                        ? "border-4 border-orange-400 shadow-lg md:shadow-xl bg-orange-50"
                        : "",
                    ].join(" ");
                    ...
                    return (
                        ...
                        onClick={() => {
                          if (isPast) {
                            toast({
                              title: " Fecha no disponible",
                              description:
                                "Solo puedes agendar desde hoy en adelante.",
                              duration: 3000,
                              variant: "destructive",
                            });
                            return;
                      ...
                      >
                        <div
                          className={`w-12 h-12 md:w-20 md:h-20 rounded-xl md:rounded-2xl flex items-center justify-center font-black text-2xl md:text-4xl ${
                            isToday
              ...
              <motion.div
                ...
                className="bg-gradient-to-br from-amber-50 to-emerald-50 rounded-2xl md:rounded-[2rem] p-6 md:p-10 shadow-xl border-4 border-green-200"
              >
                <div className="text-center space-y-6">
                  ...
                  <motion.div
                    ...
                    className="mx-auto w-20 h-20 md:w-24 md:h-24 bg-amber-500 rounded-full flex items-center justify-center shadow-lg"
                  >
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                    ...
                    >
                      <CircleAlert className="w-10 h-10 md:w-12 md:h-12 text-white stroke-[3]" />
                    </motion.div>
                  </motion.div>

                  {/* Mensaje de xito */}
                  <div className="space-y-3">
                    <h2 className="text-2xl md:text-4xl font-black text-amber-600">
                    ...
                    <p className="text-base md:text-xl text-gray-700 font-bold">
                      ...
                      , para finalizar tu reserva por favor has click en el
                      siguiente botón.
                    </p>
                  </div>
                  ...
                  <div className="bg-blue-50 p-4 md:p-5 rounded-2xl border-2 border-blue-200 max-w-2xl mx-auto space-y-3">
                    <p className="text-sm md:text-base text-gray-700 font-bold">
                      📱 Te contactaremos vía{" "}
                      <span className="text-blue-600">WhatsApp</span> al número{" "}
                    ...
                    >
                      ...
                      Termina la confirmación por WhatsApp
                    </motion.a>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                      ...
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-sm md:text-base font-black text-gray-800">
                            {confirmedBooking?.fecha}
                          </p>
                          <p className="text-base md:text-lg font-black text-green-600">
                            {formatTime12Hour(confirmedBooking?.hora)}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-4 md:p-5 rounded-2xl border-2 border-green-200 shadow-md">
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <div className="text-left">
                          <p className="text-xs font-bold text-green-500 uppercase">
                            {confirmedBooking?.numPersonas > 1
                              ? "Servicio para"
                              : "Servicio"}
                          </p>
                          <p className="text-sm md:text-base font-black text-gray-800">
                            {confirmedBooking?.numPersonas > 1
                              ? `${confirmedBooking.numPersonas} personas`
                              : confirmedBooking?.serviceType}
                          </p>
                          <p className="text-xs font-bold text-green-600">
                            Total:{" "}
                            {formatCurrency(confirmedBooking?.finalTotal || 0)}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {confirmedBooking?.hasReferral && (
                    <div className="max-w-2xl mx-auto w-full bg-slate-50 p-4 md:p-5 rounded-2xl border-2 border-slate-200 shadow-md">
                      ...
                      <p className="text-sm md:text-base font-bold text-slate-700">
                        Referido por{" "}
                        {confirmedBooking?.referrerRoleLabel || "referido"}{" "}
                        {confirmedBooking?.referrerName}
                      </p>
                  ...
                  <div className="max-w-3xl mx-auto w-full">
                    <div className="bg-white border-2 border-amber-200 rounded-2xl p-4 md:p-5 space-y-3 shadow-sm">
                      <p className="text-base md:text-lg font-black text-amber-700 flex items-center gap-2">
                        <span aria-hidden="true"></span> Recomendaciones para tu
                        visita
                      </p>
                      <p className="text-sm md:text-base font-black text-gray-800">
                        Si tienes dudas o cambios escríbenos al WhatsApp{" "}
                        <span className="text-amber-600">
                          {BUSINESS_WHATSAPP_NUMBER}
                        </span>
                        .
                      ...
                      <div className="space-y-2 text-sm md:text-base text-gray-700 font-bold">
                        <p className="text-amber-700 font-black">
                          Cómo prepararte para recibir al piojólogo certificado
                        </p>
                        <ul className="list-disc list-inside space-y-1">
                          <li>
                            Cabello seco, limpio y sin productos; lávalo el día
                            anterior y llega con el cabello totalmente seco.
                          </li>
                          <li>
                            Cabello desenredado para facilitar la extracción.
                          </li>
                          <li>
                            No aplicar tratamientos antipiojos antes del
                            servicio.
                          </li>
                          <li>
                            Ten un espacio cómodo y una toalla limpia para los
                            hombros.
                          </li>
                          <li>
                            Informa si hay condiciones dermatológicas o
                            alergias.
                          </li>
                          <li>
                            El procedimiento puede tomar entre 30 y 60 minutos.
                          </li>
                          <li>
                            Menores de edad deben estar acompañados por un
                            adulto responsable.
                          </li>
                        </ul>
                      </div>
                      <div className="space-y-2 text-sm md:text-base text-gray-700 font-bold">
                        ...
                        <ul className="list-disc list-inside space-y-1">
                          ...
                          <li>
                            ...
                            (usa agua caliente si es posible).
                          </li>
                          <li>
                            Lava y desinfecta peines, cepillos, ligas, gorras y
                            diademas.
                          </li>
                          <li>
                            Evita compartir objetos de cabeza (peines,
                            almohadas, audífonos, bufandas, gorras).
                          </li>
                          <li>
                            Aspira sillones, almohadas, colchones y asientos del
                            vehículo como medida adicional.
                          </li>
        ...
        >
          <DialogContent
            className={`text-[20px] md:text-xl leading-relaxed md:leading-normal bg-gradient-to-b from-blue-50 to-white font-sans ${
              isEmbeddedMobile
          ...
          >
            <DialogHeader className="pt-8 pb-6 text-center bg-gradient-to-b from-blue-100 to-blue-50 shrink-0">
              <div className="flex items-center justify-center gap-3 mb-2">
                ...
                />
                <DialogTitle
                  className="font-lobster-two text-3xl md:text-4xl font-bold uppercase text-blue-700 tracking-wide"
                  style={{ WebkitTextStroke: "1px rgba(30, 64, 175, 0.25)" }}
                >
                  Agenda tu Cita
                </DialogTitle>
              </div>
                ...
                {!showForm ? (
                  ...
                  <div className="space-y-4 pt-2">
                    <div className="text-center space-y-2">
                      ...
                      <p className="font-lobster-two text-2xl md:text-3xl font-bold text-gray-800">
                                ...
                                if (requireAdvance12h) {
                                  ...
                                  if (
                                    ...
                                    toast({
                                      ...
                                      description:
                                        "Los servicios se solicitan con mínimo 12 horas de antelación.",
                                      duration: 4000,
                                      variant: "destructive",
                                    });
                                    return;
                  ...
                  <form
                    ...
                    className="space-y-5 md:space-y-6 text-[20px] md:text-xl pt-2 pb-4"
                  ...
                  >
                    ...
                    {/* Datos del Cliente */}
                    <div className="space-y-3">
                      <p className="font-lobster-two text-2xl md:text-3xl font-bold text-gray-700">
                        👤 Datos del Cliente
                      </p>
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const newServicesPerPerson = Array(numPersonas)
                              .fill(null)
                              .map(
                                (_, idx) =>
                      ...
                      <div className="space-y-1">
                        <label className="text-base md:text-lg font-bold text-gray-700 ml-2 mb-1 block">
                          🎂 Edad de la(s) Persona(s) que Recibirán el Servicio
                          *
                        </label>
                        <input
                          required
                      ...
                      <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border-2 border-slate-200">
                        <p className="text-base md:text-lg font-black text-slate-700 uppercase tracking-wide">
                          💆 Nivel de Infestación por Persona
                        </p>
                        {Array(parseInt(form.numPersonas) || 1)
                          .fill(null)
                          .map((_, idx) => (
                            <div key={idx} className="space-y-1">
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const newServices = [
                                    ...form.servicesPerPerson,
                                  ];
                                  newServices[idx] = e.target.value;
                                  setForm({
                                    ...form,
                                    servicesPerPerson: newServices,
                                  });
                                }}
                              >
                        ...
                        <div className="space-y-2">
                          <div className="bg-gradient-to-r from-green-500 to-emerald-500 p-4 rounded-xl shadow-lg border-2 border-green-400">
                            <div className="flex items-center justify-between">
                              <span className="text-white font-black text-base md:text-lg uppercase">
                                💰 Total del Servicio:
                              </span>
                    ...
                    <div className="bg-[#eef5fb] p-4 rounded-2xl border-2 border-[#c8dceb] space-y-3">
                      <p className="font-lobster-two text-2xl md:text-3xl font-bold text-blue-600">
                        Contacto
                      </p>
                      <div className="space-y-1">
                        <label className="text-base md:text-lg font-bold text-gray-700 ml-2 mb-1 block">
                      ...
                      {sellerReferralContext.isActive ? (
                        <div className="space-y-2 rounded-2xl border-2 border-emerald-200 bg-emerald-50 p-4">
                          <p className="text-base md:text-lg font-black text-emerald-700">
                            Peluquería vinculada al link
                          </p>
                          <p className="text-sm md:text-base font-bold text-gray-700">
                            Peluquería:{" "}
                            <span className="text-emerald-700">
                              {sellerReferralContext.businessName}
                            </span>
                          </p>
                          <p className="text-sm md:text-base font-bold text-gray-700">
                            Vendedor:{" "}
                            <span className="text-emerald-700">
                              {sellerReferralContext.sellerName}
                            </span>
                          </p>
                          <p className="text-xs md:text-sm font-bold text-emerald-700">
                            Este agendamiento quedará asociado automáticamente a
                            esa peluquería y a ese vendedor.
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-1">
                          ...
                          {referralValidation.message && (
                            <p
                              className={`text-sm font-bold ml-2 mt-1 flex items-center gap-1 ${
                                referralValidation.isValid
                                  ? "text-green-600"
                                  : "text-red-600"
                              }`}
                        ...
                        <AddressAutocomplete
                          ...
                          onSelect={(suggestion) => {
                            ...
                            toast({
                              ...
                              className:
                                "bg-red-50 border-2 border-red-200 text-red-700 rounded-2xl font-bold",
                            });
                          }}
                        />
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleChange("descripcionUbicacion", e.target.value)
                          }
                    ...
                    <div className="bg-[#f8f3f3] p-4 rounded-2xl border-2 border-[#e2d7d7] space-y-3">
                      <p className="font-lobster-two text-2xl md:text-3xl font-bold text-red-600">
                        Datos de Salud
                      </p>
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          onChange={(e) =>
                            handleChange("hasAlergias", e.target.checked)
                          }
                          className="w-5 h-5 rounded border-2 border-red-300"
                        />
                        <label
                          htmlFor="hasAlergias"
                          className="font-bold text-gray-700 cursor-pointer text-base md:text-lg"
                        >
                      ...
                      {form.hasAlergias && (
                        <textarea
                          ...
                          onChange={(e) =>
                            handleChange("detalleAlergias", e.target.value)
                          }
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border-2 border-slate-200 space-y-3">
                      <p className="font-lobster-two text-2xl md:text-3xl font-bold text-slate-700">
                        Referencias
                      </p>
                      <div className="space-y-1">
                        <label className="text-base md:text-lg font-bold text-gray-700 ml-2 mb-1 block">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleChange("referidoPor", e.target.value)
                          }
                    ...
                    <div className="bg-amber-50 p-4 rounded-2xl border-2 border-amber-200 space-y-3">
                      <p className="font-lobster-two text-2xl md:text-3xl font-bold text-amber-700">
                        Pago
                      </p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <button
                          type="button"
                          onClick={() =>
                            handleChange("paymentMethod", "pay_now")
                          }
                        ...
                        >
                          ...
                          <p className="text-sm text-gray-600 font-bold mt-1">
                            Genera un link dinámico con Bold por{" "}
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleChange("paymentMethod", "pay_later")
                          }
                          className={`w-full text-left rounded-xl md:rounded-2xl border-2 p-4 font-black transition-all ${
                      ...
                      {form.paymentMethod === "pay_now" && (
                        <div className="bg-white border-2 border-amber-200 rounded-2xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                          <div className="space-y-1 flex-1">
                            <p className="text-xs uppercase tracking-wide text-amber-600 font-black">
                            ...
                            {hasOpenedBoldCheckout ? (
                              <div className="bg-green-50 border-2 border-green-200 rounded-lg p-2 text-green-700 text-xs font-black inline-flex items-center gap-2 mt-1">
                                ✅ Link de pago abierto. Ya puedes confirmar la
                                reserva.
                              </div>
                            ) : (
                              <>
                                <p className="text-xs text-gray-600 font-bold">
                                  Se abrirá una pestaña con el checkout de Bold.
                                </p>
                                <p className="text-xs text-orange-600 font-black">
                                  Debes abrir Bold para habilitar la
                                  confirmación.
                                </p>
                              </>
                            )}
                    ...
                    {/* Términos */}
                    <div
                      className={`p-4 rounded-2xl border-2 ${fieldErrors.terminosAceptados ? "bg-red-50 border-red-300" : "bg-green-50 border-green-200"}`}
                    >
                      <div className="flex items-start gap-3">
                        <input
                          ...
                          onChange={(e) =>
                            handleChange("terminosAceptados", e.target.checked)
                          }
                        ...
                        <label
                          ...
                          className="font-bold text-gray-700 cursor-pointer text-xs md:text-sm"
                        >
                          ✅ Acepto los términos y condiciones del servicio
                          junto con la política de bioseguridad.
                        </label>
                      </div>
                    ...
                    <p className="text-sm md:text-base text-gray-500 font-bold text-center">
                      Al reservar aceptas ser contactado por WhatsApp para
                      confirmar la visita.
                    </p>
                  </form>
                )}
            ...
            {showForm && (
              <div
                className={`${isEmbeddedMobile ? "" : "shrink-0"} border-t border-blue-200 bg-white px-4 md:px-6 lg:px-8 py-4 shadow-[0_-8px_24px_rgba(59,130,246,0.12)]`}
              >
                <Button
                  ...
                  disabled={
                    isSubmitting ||
                    (form.paymentMethod === "pay_now" && !hasOpenedBoldCheckout)
                  }
