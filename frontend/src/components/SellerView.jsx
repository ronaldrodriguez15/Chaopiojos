...
import { Building2, Upload, FileText, Briefcase, Clock3, RefreshCw, Phone, MapPin, Wallet, Copy, Users, Eye, Link2 } from 'lucide-react';
...
const statusConfig = {
  pending_review: { label: 'Pendiente revisión', badge: 'bg-amber-100 text-amber-700 border-amber-200' },
...
const SellerView = ({ currentUser }) => {
  ...
  const approvedReferrals = useMemo(
    () => referrals.filter((item) => item.status === 'approved' && item.booking_link),
    [referrals]
  );

  const cards = useMemo(() => ([
    { label: 'Contratos registrados', value: stats.total || 0, tone: 'bg-blue-50 border-blue-200 text-blue-700', icon: Briefcase },
    { label: 'Pendientes revisión', value: stats.pending_review || 0, tone: 'bg-amber-50 border-amber-200 text-amber-700', icon: Clock3 },
  ...
  const handleCopyBookingUrl = async (relativePath) => {
    ...
    try {
      ...
      toast({
        title: 'Link copiado',
        description: 'El enlace de esta peluquería quedó copiado.',
  ...
  const validateForm = () => {
    const errors = {};
    if (!formData.business_name.trim()) errors.business_name = 'El nombre de la peluquería es obligatorio';
    if (!formData.contact_name.trim()) errors.contact_name = 'El nombre del contacto es obligatorio';
    if (!formData.whatsapp.trim() && !formData.phone.trim()) errors.whatsapp = 'Debes registrar al menos un teléfono o WhatsApp';
  ...
  const handleSubmit = async (event) => {
    ...
    if (!validateForm()) {
      toast({
        title: 'Faltan datos',
        description: 'Revisa los campos marcados antes de guardar el contrato.',
        variant: 'destructive',
        className: 'rounded-3xl border-4 border-red-200 bg-red-50 text-red-600 font-bold'
      });
      return;
    ...
    if (!result.success) {
      ...
      toast({
        title: 'Error',
        description: result.message || 'No se pudo registrar el contrato referido',
        variant: 'destructive',
        className: 'rounded-3xl border-4 border-red-200 bg-red-50 text-red-600 font-bold'
      });
      return;
    ...
    toast({
      ...
      description: 'La peluquería quedó cargada en tus referidos.',
      className: 'bg-green-100 text-green-800 rounded-2xl border-2 border-green-200'
    });
    await loadDashboard();
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm md:text-base font-bold text-cyan-600">{currentUser?.name} • contratos y peluquerías referidas</p>
              </div>
            </div>
        ...
        <TabsContent value="panel" className="space-y-6">
          <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-6">
            <div className="bg-white rounded-[2rem] border-4 border-cyan-100 p-6 shadow-lg">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-black text-gray-800">Links de agendamiento por peluquería</h3>
              </div>
              {approvedReferrals.length === 0 ? (
                <div className="rounded-2xl border-2 border-dashed border-cyan-200 bg-cyan-50 p-8 text-center font-bold text-cyan-700">Aún no tienes peluquerías aprobadas con link activo.</div>
              ) : (
                <div className="space-y-3">
                  {approvedReferrals.map((item) => (
            ...
            <div className="bg-gradient-to-br from-cyan-500 to-blue-600 rounded-[2rem] p-6 shadow-lg text-white border-4 border-cyan-300">
              <p className="text-xs uppercase tracking-[0.25em] font-black opacity-80">Ganancias por links</p>
              <h3 className="text-3xl font-black mt-3">{formatCurrency(earnings.summary?.total_amount || 0)}</h3>
              <p className="text-sm md:text-base font-bold text-cyan-50 mt-3 leading-relaxed">Seguimiento de reservas creadas con tus links por peluquería y pago por cabeza agendada.</p>
              <div className="mt-6 space-y-3">
                ...
                <div className="rounded-2xl bg-white/15 border border-white/20 p-4 flex items-center justify-between"><span className="font-bold">Reservas con link</span><span className="text-2xl font-black">{earnings.summary?.bookings_count || 0}</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2rem] border-4 border-cyan-100 p-6 shadow-lg">
            ...
            {earnings.commissions?.length ? (
                ...
                {earnings.commissions.map((item) => (
                  <div key={item.id} className="rounded-2xl border-2 border-cyan-100 bg-cyan-50/60 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                      ...
                      {item.booking?.seller_referral_name && <p className="text-sm font-bold text-cyan-700 mt-1">Peluquería: {item.booking.seller_referral_name}</p>}
              ...
              <div className="rounded-2xl border-2 border-dashed border-cyan-200 bg-cyan-50 p-8 text-center font-bold text-cyan-700">Aún no tienes reservas generadas con tus links.</div>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="referrals" className="space-y-6">
          <div className="grid grid-cols-1 xl:grid-cols-[0.95fr_1.05fr] gap-6">
            <div className="bg-white rounded-[2rem] border-4 border-cyan-100 p-6 shadow-lg">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div><label className="block text-sm font-bold text-gray-600 mb-2">Contacto principal *</label><input className={`w-full rounded-2xl border-2 p-4 font-bold outline-none ${fieldErrors.contact_name ? 'border-red-300 bg-red-50' : 'border-cyan-200 bg-cyan-50 focus:border-cyan-400'}`} value={formData.contact_name} onChange={(e) => handleInputChange('contact_name', e.target.value)} placeholder="Persona de contacto" /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-bold text-gray-600 mb-2">Teléfono</label><input className="w-full rounded-2xl border-2 border-cyan-200 bg-cyan-50 p-4 font-bold outline-none focus:border-cyan-400" value={formData.phone} onChange={(e) => handleInputChange('phone', e.target.value)} placeholder="Número fijo o celular" /></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-bold text-gray-600 mb-2">Correo</label><input type="email" className="w-full rounded-2xl border-2 border-cyan-200 bg-cyan-50 p-4 font-bold outline-none focus:border-cyan-400" value={formData.email} onChange={(e) => handleInputChange('email', e.target.value)} placeholder="contacto@peluqueria.com" /></div>
                ...
                <div><label className="block text-sm font-bold text-gray-600 mb-2">Observaciones</label><textarea rows={4} className="w-full rounded-2xl border-2 border-cyan-200 bg-cyan-50 p-4 font-bold outline-none focus:border-cyan-400 resize-none" value={formData.notes} onChange={(e) => handleInputChange('notes', e.target.value)} placeholder="Notas comerciales, condiciones o comentarios relevantes" /></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-[2rem] border-4 border-cyan-100 p-6 shadow-lg">
              ...
              {referrals.length === 0 ? (
                <div className="rounded-2xl border-2 border-dashed border-cyan-200 bg-cyan-50 p-10 text-center text-cyan-700 font-bold">Todavía no has registrado peluquerías.</div>
              ) : (
                    ...
                    return (
                      <div key={item.id} className="rounded-[1.75rem] border-2 border-cyan-100 bg-cyan-50/60 p-5">
                        ...
                        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm font-semibold text-gray-700">
                          <div className="bg-white rounded-xl border border-cyan-100 p-3 flex items-start gap-2"><Phone className="w-4 h-4 mt-0.5 text-cyan-600" /><div><p>{item.whatsapp || item.phone || 'Sin teléfono registrado'}</p>{item.email && <p className="text-xs text-gray-500 mt-1">{item.email}</p>}</div></div>
                        ...
                        {item.status === 'approved' && item.booking_link && (
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {item.chamber_of_commerce_url && <a href={item.chamber_of_commerce_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-white border-2 border-cyan-200 px-3 py-2 text-xs font-black text-cyan-700 hover:bg-cyan-100"><FileText className="w-4 h-4" />Cámara de Comercio</a>}
                          {item.rut_url && <a href={item.rut_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-white border-2 border-cyan-200 px-3 py-2 text-xs font-black text-cyan-700 hover:bg-cyan-100"><FileText className="w-4 h-4" />RUT</a>}
                        </div>
            ...
            {selectedReferral && (
              <div className="max-h-[80vh] overflow-y-auto">
                ...
                <div className="px-6 md:px-8 pb-8 space-y-4">
                  ...
                  {selectedReferral.status === 'approved' && selectedReferral.booking_link && <div className="bg-white rounded-xl border-2 border-cyan-100 p-4"><p className="text-xs font-black text-cyan-600 uppercase mb-2">Link de agendamiento</p><p className="text-sm font-bold text-gray-700 break-all">{buildBookingUrl(selectedReferral.booking_link)}</p></div>}
                </div>
              </div>
            )}
          </DialogContent>
